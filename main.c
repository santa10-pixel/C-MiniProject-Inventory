...
int getProductId(int index) {
    if (index >= 0 && index < productCount) return inventory[index].productID;
    return -1;
}

EMSCRIPTEN_KEEPALIVE
const char* getProductName(int index) {
    if (index >= 0 && index < productCount) return inventory[index].name;
    return "";
}

EMSCRIPTEN_KEEPALIVE
float getProductPrice(int index) {
    if (index >= 0 && index < productCount) return inventory[index].price;
    return -1.0f;
}

EMSCRIPTEN_KEEPALIVE
int getProductQuantity(int index) {
    if (index >= 0 && index < productCount) return inventory[index].quantity;
    return -1;
}
